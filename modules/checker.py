...
class checker:
    ...
    def check(self, token):
        ...
        if state.status_code == 200:
            log.info('Checker', f'{token[:30]}... >> Valid >> Token info è solo per i premium')
            ...
            log.warn('Checker', f'{token[:30]}... >> CLOUDFLARE BLOCCATO >>Aspetta 5 secondi e riprova')
            time.sleep(5)
            self.check(token)
        ...
        elif 'Devi verificarti cojone' in state.text:
    ...
    def main(self):
        savevalid = ui().ask('Salva solo quelli validi', True)
        thread(
            files().getthreads(),
            self.check,
            files().gettokens(),
