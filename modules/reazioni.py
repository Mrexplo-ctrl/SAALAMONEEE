...
class reaction:
    ...
    def bypass(self, token):
        ...
        if r.status_code == 204:
            ...
            log.warn('Reaction', f'{token[:30]}... >> CLOUDFLARE BLOCCATO >> aspetta 5 secondi e riprova')
            time.sleep(5)
            self.bypass(token)
    ...
    def debypass(self, token):
        ...
        if r.status_code == 204:
            ...
            log.warn('De reaction', f'{token[:30]}... >> CLOUDFLARE BLOCCATO >> aspetta 5 secondi e riprova')
            time.sleep(5)
            self.bypass(token)
